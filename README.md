...
4. oRPC mutations re-validate permissions server-side
...
While Electric handles reads, mutations go through oRPC with additional authorization:
...
delete: authedProcedure
  ...
  .handler(async ({ context, input }) => {
    // Find the todo first
    const [todo] = await context.db.select().from(todosTable).where(eq(id, input.id))

    // Verify the user has access
    if (!todo.user_ids.includes(context.session.user.id)) {
...
3. **Add ownership checks** in your oRPC mutations
...
### 4. Add an oRPC router

Create `src/lib/orpc/categories.ts`:
    ...
    .handler(async ({ context, input }) => {
      const result = await context.db.transaction(async (tx) => {
        ...
        const [newItem] = await tx
          ...
          .values({ ...input, user_id: context.session.user.id })
          .returning()
...
### 5. Wire up the oRPC router

Add to `src/routes/api/orpc/$.ts`:

```tsx
import { categoriesRouter } from "@/lib/orpc/categories"
...
export const categoriesCollection = createCollection(
  electricCollectionOptions({
    ...
    onInsert: async ({ transaction }) => {
      ...
      const result = await orpc.categories.create({
...
That's it! Your new table is now fully integrated with Electric sync, oRPC mutations, and TanStack DB queries.
...
#### oRPC Integration for Mutations

This starter uses [oRPC](https://orpc.dev/) for type-safe mutations while Electric handles real-time reads:
...
The collection hooks use oRPC for all mutations, providing full end-to-end type safety:
...
- `/api/orpc/*` - oRPC mutations with full type safety
...
- **Use Electric for reads** - `useLiveQuery` with collections, not oRPC queries
- **Use collection operations for writes** - Call `collection.insert()`, not `orpc.create()` directly
- **Preload collections in route loaders** - Prevents loading flicker and ensures data availability

#### Why These Rules Matter

- **Electric handles reads** - Direct oRPC reads bypass real-time sync and optimistic updates
